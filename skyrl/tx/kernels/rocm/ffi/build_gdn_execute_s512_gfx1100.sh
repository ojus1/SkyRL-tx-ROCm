#!/usr/bin/env bash
# Build the exact S512 execute-only typed FFI for gfx1100 without opening a GPU.
...
readonly EXPECTED_BASENAME="libskyrl_gdn_execute_s512_gfx1100.so"
readonly EXPECTED_SYMBOL="skyrl_gdn_execute_s512_f32_bf16_v1"
readonly COMPILATION_UNIT_ID="736b79726c5f6764"
readonly SCRIPT_DIR="$(cd -- "$(dirname -- "${BASH_SOURCE[0]}")" && pwd -P)"
readonly REPO_ROOT="$(cd -- "$SCRIPT_DIR/../../../../.." && pwd -P)"
readonly SOURCE="$SCRIPT_DIR/gdn_execute_s512.hip"

if [[ "$REQUESTED_ARCH" != "$EXPECTED_ARCH" ]]; then
  printf 'refusing architecture %q: gdn_execute_s512 is fixed to %s\n' \
...
if [[ ! -f "$SOURCE" || -L "$SOURCE" ]]; then
  printf 'gdn_execute_s512 source is missing or is a symbolic link\n' >&2
  exit 2
fi
...
BUILD_OUTPUT="$(mktemp --suffix=.so -- "$CANONICAL_PARENT/.gdn-execute-s512.XXXXXX")"
...
"$HIPCC" -std=c++17 -O2 -fno-fast-math -fPIC -shared \
  -Wall -Wextra -Werror "--offload-arch=$EXPECTED_ARCH" \
  "-cuid=$COMPILATION_UNIT_ID" \
  "-ffile-prefix-map=$REPO_ROOT=/skyrl" \
  -isystem "$JAX_INCLUDE" -Wl,--build-id=sha1 -Wl,-z,defs \
  -Wl,-z,relro -Wl,-z,now "$SOURCE" -o "$BUILD_OUTPUT"
