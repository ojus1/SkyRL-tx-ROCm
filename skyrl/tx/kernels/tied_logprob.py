...
``M x min(V, vocab_superblock_size)``.  Its two-pass logsumexp is chosen to stay
close to JAX's current ``jax.nn.logsumexp`` reduction semantics.  It recomputes
each vocabulary tile and is therefore a precision/memory oracle only; it must
not be used to claim speed.  A production kernel should use a bounded online
reduction and tiled backward without routing through these portable JAX loops.
...
def _chunk_forward_with_normalizer(
    ...
    """Return logprobs plus the max and rounded denominator needed by VJP."""
    ...
    def max_and_target_scan(
        carry: tuple[jax.Array, jax.Array],
        block_values: tuple[jax.Array, jax.Array],
    ) -> tuple[tuple[jax.Array, jax.Array], None]:
        running_max, target_logits = carry
        ...
        running_max = jnp.maximum(running_max, jnp.max(block_logits, axis=-1))
        ...
        return (running_max, target_logits), None

    (global_max, target_logits), _ = jax.lax.scan(
        max_and_target_scan,
        (initial_max, initial_target),
    ...
    finite_max = jnp.where(jnp.isfinite(global_max), global_max, 0)

    def denominator_scan(
        running_sum_f32: jax.Array,
        block_values: tuple[jax.Array, jax.Array],
        ...
        shifted_exponentials = jnp.exp(block_logits - finite_max[:, None])
        # JAX promotes BF16/F16 reductions to FP32 internally, then casts the
        # result back before log().  Retain FP32 across superblocks and perform
        # that cast exactly once after the split reduction.
        block_sum_f32 = jnp.sum(shifted_exponentials.astype(jnp.float32), axis=-1)
        return running_sum_f32 + block_sum_f32, None

    denominator_f32, _ = jax.lax.scan(
        denominator_scan,
