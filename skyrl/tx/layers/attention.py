...
def _can_use_rocm_pallas_attention(
    ...
    if q.ndim != 4 or k.ndim != 4 or v.ndim != 4 or attention_mask.ndim != 2:
        return False
    if q.dtype not in _PALLAS_SUPPORTED_DTYPES or k.dtype != q.dtype or v.dtype != q.dtype:
        return False
    if not (
    ...
    if num_kv_heads != num_v_heads or num_q_heads % num_kv_heads != 0:
        return False
    if num_q_heads != _PALLAS_SUPPORTED_Q_HEADS or num_kv_heads != _PALLAS_SUPPORTED_KV_HEADS:
        return False
...
def _is_large_causal_self_attention(q: jax.Array, k: jax.Array, v: jax.Array, is_causal: bool) -> bool:
...
def _pallas_mha(
    ...
    required_parameters = {"block_sizes", "backward_pass_impl", "num_warps", "num_stages"}
    missing_parameters = required_parameters.difference(inspect.signature(mha).parameters)
