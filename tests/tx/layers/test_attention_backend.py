...
def test_has_cuda_backend_for_cuda(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "get_backend",
        lambda: SimpleNamespace(platform_version="CUDA 12.8"),
    )
...
def test_dot_product_attention_preserves_cuda_cudnn_precedence(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: pytest.fail(
            "ROCm selector must not run after a supported CUDA dispatch"
        ),
    )
    ...
    monkeypatch.setattr(
        attention.jax.nn, "dot_product_attention", fake_dot_product_attention
    )
...
def test_dot_product_attention_uses_generic_fallback_for_ineligible_input(monkeypatch):
    ...
    monkeypatch.setattr(attention, "_has_cuda_backend", lambda: False)
    monkeypatch.setattr(
        attention, "_can_use_rocm_pallas_attention", lambda *args: False
    )
    ...
    np.testing.assert_array_equal(
        calls[0][1]["mask"], mask[:, None, None, :].astype(bool)
    )
...
def test_explicit_pallas_opt_in_refuses_long_quadratic_fallback(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda *args, **kwargs: pytest.fail(
            "long opt-in input must not reach quadratic XLA attention"
        ),
    )
...
def test_default_off_rocm_refuses_long_quadratic_fallback(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "dot_product_attention",
        lambda *args, **kwargs: pytest.fail(
            "long default-off input must not reach quadratic XLA attention"
        ),
    )
...
def test_explicit_pallas_opt_in_refuses_sequence_above_gfx1100_safety_limit(
    monkeypatch,
):
    ...
    with pytest.raises(
        ValueError, match="exceeds the validated 16384-token safety limit"
    ):
        attention.dot_product_attention(q, k, v, mask, True, 256)
...
def test_rocm_safety_limit_applies_even_when_pallas_is_disabled(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        "dot_product_attention",
        lambda *args, **kwargs: pytest.fail(
            "unsafe 32K ROCm input must not reach the generic fallback"
        ),
    )
...
def test_rocm_pallas_attention_expands_gqa_and_uses_mask_as_segment_ids(monkeypatch):
    ...
    with jax.default_device(cpu):
        ...
        k = jnp.broadcast_to(
            jnp.asarray([10, 20], dtype=jnp.float16)[None, None, :, None],
            (1, 64, 2, 64),
        )
        v = jnp.broadcast_to(
            jnp.asarray([30, 40], dtype=jnp.float16)[None, None, :, None],
            (1, 64, 2, 64),
        )
        ...
        def fake_pallas_mha(q_arg, k_arg, v_arg, segment_ids_arg, scale_arg):
            captured.update(
                q=q_arg, k=k_arg, v=v_arg, segment_ids=segment_ids_arg, scale=scale_arg
            )
            return q_arg
    ...
    np.testing.assert_array_equal(
        np.asarray(captured["k"][0, 0, :, 0]), [10, 10, 20, 20]
    )
    np.testing.assert_array_equal(
        np.asarray(captured["v"][0, 0, :, 0]), [30, 30, 40, 40]
    )
    np.testing.assert_array_equal(
        np.asarray(captured["segment_ids"]), np.asarray(mask != 0)
    )
...
def test_pallas_mha_uses_validated_rocm_compiler_contract(monkeypatch):
    ...
    # This test validates only the MHA argument contract. The dedicated FP32
    # delta tests exercise installation and restore the process-global private
    # JAX function; avoid leaking that mutation into later tests here.
    monkeypatch.setattr(
        attention, "_install_pallas_fp32_delta_preprocess_patch", lambda: False
    )
    ...
    fake_mha.__signature__ = __import__("inspect").signature(
        lambda q, k, v, segment_ids, *, block_sizes, backward_pass_impl, num_warps, num_stages, **kwargs: (
            None
        )
    )
...
def test_pallas_mha_interpret_forward_and_gradients_match_reference():
    ...
    with jax.default_device(cpu):
        ...
        actual_grads = jax.grad(
            lambda *args: jnp.sum(actual(*args) ** 2), argnums=(0, 1, 2)
        )(q, k, v)
        reference_grads = jax.grad(
            lambda *args: jnp.sum(reference(*args) ** 2), argnums=(0, 1, 2)
        )(q, k, v)
        valid_query_mask = segment_ids[:, :, None, None].astype(jnp.float32)
        actual_masked_grads = jax.grad(
            lambda *args: jnp.sum(actual(*args) ** 2 * valid_query_mask),
            argnums=(0, 1, 2),
        )(q, k, v)
        generic_masked_grads = jax.grad(
            lambda *args: jnp.sum(generic(*args) ** 2 * valid_query_mask),
            argnums=(0, 1, 2),
        )(q, k, v)

    np.testing.assert_allclose(
        np.asarray(actual_out), np.asarray(reference_out), rtol=2e-5, atol=2e-5
    )
    np.testing.assert_allclose(
        np.asarray(actual_out[:, :6]),
        np.asarray(generic_out[:, :6]),
        rtol=2e-5,
        atol=2e-5,
    )
    for actual_grad, reference_grad in zip(actual_grads, reference_grads, strict=True):
        np.testing.assert_allclose(
            np.asarray(actual_grad), np.asarray(reference_grad), rtol=3e-5, atol=3e-5
        )
    for actual_grad, generic_grad in zip(
        actual_masked_grads, generic_masked_grads, strict=True
    ):
        np.testing.assert_allclose(
            np.asarray(actual_grad), np.asarray(generic_grad), rtol=3e-5, atol=3e-5
        )
